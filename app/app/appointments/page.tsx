...
export default function AppointmentsPage() {
  ...
  const appointments = [
    {
      id: 1,
      doctor: "Dr. Ní Bhraonáin",
      specialty: "Cardiology",
      date: "15 Jan 2026",
      time: "09:20 AM",
      location: "St. James's Campus, Dublin 8",
      status: "Confirmed",
      statusColor: "emerald"
    },
    {
      ...
      date: "22 Jan 2026",
      time: "02:30 PM",
      location: "Mater Hospital, Dublin 7",
      status: "Pending",
      statusColor: "amber"
    },
    {
      ...
      <aside className="hidden w-72 border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 lg:flex flex-col">
        ...
        <nav className="flex-1 space-y-1 px-4">
          {[
            ...
            { label: "Appointments", icon: Calendar, href: "/app/appointments", active: true },
        ...
        <main className="flex-1 overflow-y-auto p-10 space-y-10">
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-sm font-medium text-slate-400 dark:text-slate-500">Manage and view all your appointments</p>
            </div>
            <button className="flex items-center gap-2 rounded-2xl bg-blue-600 px-6 py-3.5 text-sm font-black text-white shadow-lg dark:shadow-none shadow-blue-100 hover:bg-blue-700 transition-all">
          ...
          <div className="space-y-4">
            {appointments.map((appointment) => (
              <div key={appointment.id} className="rounded-[2.5rem] border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-[0_8px_30px_rgb(0,0,0,0.02)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.3)] transition-all hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] dark:hover:shadow-[0_8px_30px_rgb(0,0,0,0.4)]">
                <div className="flex flex-col gap-6 md:flex-row md:items-center">
                  ...
                  <div className="flex-1 space-y-2">
                    <div className="flex items-center gap-3 flex-wrap">
                      <span className={`text-[10px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${
                        appointment.statusColor === "emerald" 
                          ? "text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-900/30" 
                          : appointment.statusColor === "amber"
                    ...
                    <h4 className="text-2xl font-black text-slate-800 dark:text-slate-100">{appointment.doctor}</h4>
                    <div className="flex flex-wrap gap-6 text-sm font-bold text-slate-400 dark:text-slate-500">
                      <span className="flex items-center gap-2"><Calendar className="h-4 w-4" /> {appointment.date}</span>
                  ...
                  <div className="flex gap-3">
                    <button className="rounded-2xl border-2 border-slate-50 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-700/50 px-6 py-3 text-sm font-black text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-600 transition-all">
                      Reschedule
                    </button>
                    <button className="rounded-2xl border-2 border-blue-50 dark:border-blue-900/30 bg-blue-50/50 dark:bg-blue-900/30 px-6 py-3 text-sm font-black text-blue-600 dark:text-blue-400 hover:bg-blue-100 dark:hover:bg-blue-900/50 transition-all">
                      View Details
                    </button>
                  </div>
                </div>
