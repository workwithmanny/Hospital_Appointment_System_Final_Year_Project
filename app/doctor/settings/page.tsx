      ...
      <aside className="hidden w-72 border-r border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 lg:flex flex-col">
        ...
        <nav className="flex-1 space-y-1 px-4">
          {[
            ...
            { label: "Patients", icon: User, href: "/doctor/patients", active: false },
            { label: "Records", icon: Clipboard, href: "/doctor/records", active: false },
            { label: "Schedule", icon: Calendar, href: "/doctor/schedule", active: false },
            { label: "Settings", icon: User, href: "/doctor/settings", active: true },
          ].map((item) => (
            <Link
              key={item.label}
              href={item.href}
      ...
      <div className="flex-1 flex flex-col h-screen overflow-hidden">
        ...
        <main className="flex-1 overflow-y-auto p-10 space-y-10">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-400">Manage your account preferences and privacy</p>
          </div>

          {/* Profile Section */}
          <section className="rounded-[2.5rem] border border-slate-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-8 shadow-sm">
            ...
            <div className="space-y-6">
              <div className="flex items-center gap-6">
                <div className="h-20 w-20 rounded-3xl bg-blue-600 flex items-center justify-center text-white font-black text-2xl">
                  {fullName?.charAt(0).toUpperCase() || email?.charAt(0).toUpperCase() || "D"}
                </div>
                <div className="flex-1">
                  <p className="text-lg font-black text-slate-800 dark:text-slate-100 mb-1">{fullName || email}</p>
                  <p className="text-xs font-bold text-blue-600 dark:text-blue-400 uppercase tracking-tighter">Healthcare Provider</p>
                </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t border-slate-100 dark:border-slate-700">
                <div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 px-4 py-3">
                    ...
                    <input type="email" value={email || ""} readOnly className="flex-1 bg-transparent text-sm font-bold text-slate-800 dark:text-slate-200 outline-none" />
                  </div>
                </div>
                <div>
                  ...
                  <div className="flex items-center gap-3 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 px-4 py-3">
                    ...
                    <input type="tel" placeholder="+353 85 123 4567" className="flex-1 bg-transparent text-sm font-bold text-slate-800 dark:text-slate-200 outline-none placeholder:text-slate-400" />
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <button className="flex w-full items-center justify-between p-4 rounded-2xl border border-slate-100 dark:border-slate-700 bg-slate-50 dark:bg-slate-700 hover:bg-slate-100 dark:hover:bg-slate-600 transition-all">
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-left">
                    <p className="text-sm font-black text-slate-800 dark:text-slate-100">License Information</p>
                    <p className="text-xs font-medium text-slate-400 dark:text-slate-500">Update medical license details</p>
                  </div>
                </div>
                <ChevronRight className="h-5 w-5 text-slate-400" />
              </button>
          ...
          <div className="flex justify-end">
            <button className="flex items-center gap-2 rounded-2xl bg-blue-600 px-8 py-4 text-sm font-black text-white hover:bg-blue-700 transition-all">
